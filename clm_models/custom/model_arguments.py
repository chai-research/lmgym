...
class DataTrainingArguments:
    ...
    eval_prompt_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to prompt completions to log"}
    )

    num_eval_prompts: Optional[int] = field(
        default=1024,
        metadata={"help": "How many evaluation prompts to check"}
    )

    hellaswag_sample_size: Optional[int] = field(
        default=None,
        metadata={'help': 'How many hellaswag samples to generate metrics on'}
    )

    lambada_enabled: Optional[bool] = field(
        default=None,
        metadata={'help': 'Whether to log lambada metrics'}
    )

    add_reward_scores: Optional[bool] = field(
        default=False,
        metadata={'help': 'Whether to log reward model scores'}
    )

    clean_enabled: Optional[bool] = field(
        default=False,
        metadata={'help': 'Delete optimizer after saving checkpoint (saves space, prevents resuming)'}
    )

    eval_first_step: Optional[bool] = field(
        default=False,
        metadata={'help': 'Evaluate the model before training begin'}
    )

    train_to_probs: Optional[bool] = field(
        default=False,
        metadata={'help': 'Train against soft targets'}
    )
